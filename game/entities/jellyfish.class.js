...
export default class Pufferfish extends MovableObject {
  ...
  constructor(x, type) {
    ...
    this.type = type;
    this.SWIM_ANIMATION.path =
      "../assets/jellyfish/" + this.type + "/" + this.color + "-";
    if (this.type == "regular") {
      this.DEATH_ANIMATION.path =
        "../assets/jellyfish/" + this.type + "-dead/" + this.color + "-";
    }
  ...
  load() {
    this.loadImage(
      "../assets/jellyfish/" + this.type + "/" + this.color + "-0.png"
    );
  ...
  remove() {
    this.game.world.level.enemies.splice(
      this.game.world.level.enemies.indexOf(this),
      1
    );
  }
}
